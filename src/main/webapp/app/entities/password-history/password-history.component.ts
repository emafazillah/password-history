...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.passwordHistoryService.search({
                page: this.page - 1,
        ...
        this.passwordHistoryService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: ResponseWrapper) => this.onSuccess(res.json, res.headers),
    ...
    transition() {
        this.router.navigate(['/password-history'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: PasswordHistory) {
        return item.id;
    }
    registerChangeInPasswordHistories() {
        this.eventSubscriber = this.eventManager.subscribe('passwordHistoryListModification', (response) => this.loadAll());
    }
