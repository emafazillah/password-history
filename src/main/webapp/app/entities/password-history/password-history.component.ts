...
export class PasswordHistoryComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.passwordHistoryService
        ...
        .subscribe((res: HttpResponse<IPasswordHistory[]>) => this.paginatePasswordHistories(res.body, res.headers));
      return;
    }
    this.passwordHistoryService
      .query({
  ...
  trackId(index: number, item: IPasswordHistory) {
    return item.id;
  }
  ...
  delete(passwordHistory: IPasswordHistory) {
  ...
  protected paginatePasswordHistories(data: IPasswordHistory[], headers: HttpHeaders) {
